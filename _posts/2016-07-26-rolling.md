...
The [data.table](https://cran.r-project.org/web/packages/data.table/index.html) package in R facilitates rolling joins. I've stumbled across the term a few times so I thought I should learn about it. My first Google search for `rolling join` yielded a mix of data topics and other content; not work safe. I navigated to a few different pages and did some reading. I didn't understand what was going on though. So I created some toy examples of my own and with some tinkering I got to grips with the technique. 
